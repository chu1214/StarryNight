...
public class MoveStar : MonoBehaviourPun, IItem
{
    public float multiply_speed = 3;
    ...
    public void Use(GameObject target)
    {
        ...
        if (character != null)
        {
            character.photonView.RPC("MultiplySpeed", RpcTarget.All, multiply_speed, multiply_time);
        }
        else
        {
            // vr 캐릭터
            VRCharacterAnimationController _controller  = target.GetComponent<VRCharacterAnimationController>();
            _controller.photonView.RPC("MultiplySpeed",RpcTarget.All,multiply_speed, multiply_time);
